...
interface ResultsSectionProps {
  ...
  /** Called when user requests an export - parent handles email gating */
  onExportRequested: (format: ExportFormat) => void
}

export function ResultsSection({ findings, profile, vendors, onExportRequested }: ResultsSectionProps) {
  const { ref, isVisible } = useReveal(0.3)
  const [ctaMessage, setCtaMessage] = useState<string | null>(null)

  // Handle export request - delegates to parent for email gating
        ...
        >
          <div className="border-t border-foreground/20 pt-4 md:pt-8">
            ...
            <div className="flex flex-col gap-2 md:flex-row md:flex-wrap md:items-center md:gap-4">
              {/* Primary CTA - Download Report (EMAIL GATED) */}
              <MagneticButton
                variant="primary"
                size="lg"
            ...
            <p className="mt-3 font-mono text-xs text-foreground/40 md:mt-6">
              Enter your email to receive a HIPAA AI Risk Assessment PDF based on the results above.
            </p>
          </div>
        </div>
      </div>
