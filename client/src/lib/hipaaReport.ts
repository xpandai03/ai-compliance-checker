...
/**
 * Generates a HIPAA report as JSON and returns it as a base64 string for webhook transmission.
 */
export function generateHIPAAReportJSONBase64(report: HIPAAComplianceReport): {
  base64: string;
  filename: string;
  mimeType: string;
} {
  const timestamp = new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19);
  const sanitizedName = report.executive_summary.use_case_name.replace(/[^a-zA-Z0-9-_]/g, "_");
  const filename = `hipaa-risk-assessment_${sanitizedName}_${timestamp}.json`;
...
/**
 * Generates a HIPAA report as PDF and returns it as a base64 string for webhook transmission.
 */
export async function generateHIPAAReportPDFBase64(report: HIPAAComplianceReport): Promise<{
  base64: string;
  filename: string;
  mimeType: string;
}> {
  ...
  const drawRiskSummaryBar = (riskLevel: "HIGH" | "NEEDS_REVIEW" | "LOW") => {
    ...
    doc.rect(marginLeft, y, segmentWidth, barHeight, "F");
    doc.setFillColor(220, 210, 180);
    doc.rect(marginLeft + segmentWidth, y, segmentWidth, barHeight, "F");
    doc.setFillColor(220, 190, 190);
    doc.rect(marginLeft + segmentWidth * 2, y, segmentWidth, barHeight, "F");
  ...
  // Build PDF content (same as exportHIPAAReportAsPDF)
  addText("HIPAA AI Risk Assessment Report", 18, "bold");
  addText("Deterministic, Rules-Based Analysis", 11, "normal", [100, 100, 100]);
  addSpacer(4);
  addText(`Report ID: ${report.report_id}`, 8, "normal", [100, 100, 100]);
  ...
  // Section 7: Triggered Safeguards
  checkPageBreak(50);
  addText("7. Triggered HIPAA Safeguards", 14, "bold");
  if (report.triggered_safeguards.length === 0) {
    addText("No safeguard concerns identified.", 10, "normal", [60, 60, 60]);
